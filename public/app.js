...
(() => {
  ...
  var global$1, lookup, revLookup, Arr, inited, toString, isArray, INSPECT_MAX_BYTES, MAX_ARGUMENTS_LENGTH, INVALID_BASE64_RE, CsvError, is_object, normalize_columns_array, ResizeableBuffer, init_state, underscore, normalize_options, delimiter_discover, std, isRecordEmpty, cr, nl, boms, transform, parse;
  var init_sync = __esm({
    "node_modules/csv-parse/dist/esm/sync.js"() {
      ...
      normalize_options = function(opts) {
        ...
        if (!Array.isArray(options.delimiter)) {
          if (options.delimiter === void 0 || options.delimiter === null || options.delimiter === false) {
            options.delimiter = Buffer2.from(",", options.encoding);
          }
      ...
      delimiter_discover = function(records, options) {
        if (!options) {
          ({ delimiter_auto: options } = normalize_options({ delimiter_auto: true }));
        }
        if (typeof records === "string") {
          records = Buffer2.from(records);
        }
        if (isBuffer(records)) {
          records = ((data) => {
            const records2 = [];
          ...
          parse: function(nextBuf, end, push, close) {
            ...
            if (!delimiterDiscovered && delimiter_auto) {
              ...
              if (delimiterBufPrevious === void 0) {
                ...
                delimiterBuf = Buffer2.concat([delimiterBufPrevious, nextBuf]);
              }
              nextBuf = void 0;
  ...
  // src/validation/supplierValidator.ts
  ...
  function validateSuppliers(suppliers) {
    ...
    return {
      ...
      validRecords: suppliers.length - invalidRowNumbers.size,
      invalidRecords: invalidRowNumbers.size,
      issues
    };
  }
  var init_supplierValidator = __esm({
    "src/validation/supplierValidator.ts"() {
      "use strict";
    }
  });
  ...
  var require_app = __commonJS({
    "src/web/app.ts"() {
      ...
      var issuesDetectedElement = getElement("#issues-detected");
      var issuesContainer = getElement("#issues-container");
      function isCsvFile(file) {
        const normalizedFileName = file.name.toLowerCase();
        return normalizedFileName.endsWith(".csv") || file.type === "text/csv";
      }
      function getCsvValue(record, candidateHeaders) {
      ...
      function mapCsvRecordToSupplier(record) {
        return {
      ...
      function resetValidationIssues() {
        ...
        title.textContent = "No validation has been executed.";
      ...
      function createIssueCard(issue) {
        ...
        const supplierDisplay = issue.supplierId.trim().length > 0 ? issue.supplierId : "Not provided";
        details.textContent = `Field: ${issue.field} \xB7 Supplier ID: ${supplierDisplay}`;
      ...
      function renderValidationIssues(issues) {
        issuesContainer.replaceChildren();
        if (issues.length === 0) {
          issuesContainer.className = "empty-state success-state";
          const title = document.createElement("p");
          title.className = "empty-state-title";
          title.textContent = "No validation issues were detected.";
          const description = document.createElement("p");
          description.className = "empty-state-description";
          description.textContent = "All supplier records passed the current rules.";
          issuesContainer.append(title, description);
          return;
        }
      ...
      function handleFileSelection() {
        ...
        if (!selectedFile) {
          ...
          applicationStatus.textContent = "Select a CSV file to begin.";
          return;
        }
        selectedFileName.textContent = selectedFile.name;
        if (!isCsvFile(selectedFile)) {
          validateButton.disabled = true;
          applicationStatus.textContent = "Please select a file with the .csv extension.";
          return;
        }
        validateButton.disabled = false;
        applicationStatus.textContent = "The CSV file is ready for validation.";
      }
      async function handleValidationRequest() {
        ...
        try {
          ...
          issuesDetectedElement.textContent = String(validationResult.issues.length);
